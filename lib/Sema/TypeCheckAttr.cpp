...
#undef IGNORED_ATTR
  ...
  void visitBorrowedAttr(BorrowedAttr *attr) {
    ...
    if (D->getAttrs().hasAttribute<DynamicAttr>()) {
      diagnose(attr->getLocation(), diag::borrowed_with_objc_dynamic, ASD)
  ...
  if (auto extension = dyn_cast<ExtensionDecl>(D)) {
    ...
    if (attr->getAccess() > typeAccess) {
      diagnose(attr->getLocation(), diag::access_control_extension_more,
               typeAccess, nominal, attr->getAccess())
          .fixItRemove(attr->getRange());
      attr->setInvalid();
      return;
    }
    ...
    if (auto extAttr =
      ...
      if (attr->getAccess() > defaultAccess) {
        ...
        diagnose(attr->getLocation(), diag::access_control_ext_member_redundant,
                 attr->getAccess(), D, extAttr->getAccess())
            .fixItRemove(attr->getRange());
      }
    } else {
      if (auto VD = dyn_cast<ValueDecl>(D)) {
...
void AttributeChecker::visitSetterAccessAttr(
  ...
  if (attr->getAccess() > getterAccess) {
    ...
    diagnose(attr->getLocation(), diag::access_control_setter_redundant,
             attr->getAccess(), D, getterAccess)
        .fixItRemove(attr->getRange());
    return;
  }
}
...
void AttributeChecker::visitSPIAccessControlAttr(SPIAccessControlAttr *attr) {
  if (auto VD = dyn_cast<ValueDecl>(D)) {
    ...
    if (!canUseAttr(VD) &&
        ...
        !(DC && DC->isSPI())) {
      diagnoseAndRemoveAttr(attr, diag::spi_attribute_on_non_public, declAccess,
                            VD);
    }
  ...
  if (auto objcName = attr->getName()) {
    if (isa<ClassDecl>(D) || isa<ProtocolDecl>(D) || isa<VarDecl>(D)
      ...
      if (objcName->getNumArgs() > 0) {
        ...
        softenIfAccessNote(
            D, attr,
            diagnose(firstNameLoc, diag::objc_name_req_nullary, D)
...
visitObjCImplementationAttr(ObjCImplementationAttr *attr) {
  ...
  if (!langAttr) {
    ...
    if (auto ED = dyn_cast<ExtensionDecl>(D)) {
      auto diag = diagnose(attr->getLocation(),
                           diag::make_decl_objc_for_implementation, D);
...
SubscriptDecl::evaluateDynamicMemberLookupEligibility(const SubscriptDecl *SD,
                                                      bool ignoreLabel) {
  if (SD->isInvalid()) {
    return DynamicMemberLookupSubscriptEligibility::None;
  }
  ...
  auto dynamicMemberParam = indices->get(0);
  if (!ignoreLabel && dynamicMemberParam->getArgumentName() !=
                          SD->getASTContext().Id_dynamicMember) {
    return DynamicMemberLookupSubscriptEligibility::None;
  }

  bool first = true;
  for (auto index : *indices) {
    if (!index->hasInterfaceType() || (first && index->isVariadic()) ||
  ...
  if (TypeChecker::conformsToKnownProtocol(
          dynamicMemberParam->getTypeInContext(),
          KnownProtocolKind::ExpressibleByStringLiteral)) {
    return DynamicMemberLookupSubscriptEligibility::String;
  }

  if (getDynamicMemberParamTypeAsKeyPathType(
          dynamicMemberParam->getInterfaceType())) {
    return DynamicMemberLookupSubscriptEligibility::KeyPath;
  }
...
void AttributeChecker::visitDynamicMemberLookupAttr(
  ...
  // We first want to consider only subscript members known to match the
  // `subscript(dynamicMember:...)` name requirement, so we can partition those
  // out.
  auto firstMismatch = std::partition(
      candidates.begin(), candidates.end(), [](LookupResultEntry entry) {
        auto *SD = cast<SubscriptDecl>(entry.getValueDecl());
        auto *indices = SD->getIndices();
        return indices->size() > 0 && indices->get(0)->getArgumentName() ==
                                          SD->getASTContext().Id_dynamicMember;
      });

  if (candidates.begin() < firstMismatch) {
    // We have at least one candidate whose name matches
    // `subscript(dynamicMember:...)`; if any of these candidates satisfy the
    // requirement, we don't need to diagnose any mismatches.
    DiagnosticTransaction transaction(Ctx.Diags);
    for (auto *it = candidates.begin(); it < firstMismatch; ++it) {
      auto *SD = cast<SubscriptDecl>(it->getValueDecl());
  ...
  } else if (firstMismatch < candidates.end()) {
    // We didn't find any subscripts whose name matches
    // `subscript(dynamicMember:...)`, but we might be able to offer fix-its for
    // subscripts which would be valid if not for a missing `dynamicMember`
    // argument label.
    for (auto *it = firstMismatch; it < candidates.end(); ++it) {
      auto *SD = cast<SubscriptDecl>(it->getValueDecl());
      diagnose(SD, diag::invalid_dynamic_member_lookup_type, type);

      auto *indices = SD->getIndices();
      if (indices->size() > 0) {
        auto param = indices->get(0);
        if (param->getParameterNameLoc().isValid() &&
            param->getArgumentNameLoc().isInvalid() &&
            SubscriptDecl::evaluateDynamicMemberLookupEligibility(
                SD, /*ignoreArgLabel=*/true) !=
                DynamicMemberLookupSubscriptEligibility::None) {
    ...
    // We didn't have any matches at all.
  ...
  if (attr->isNoAsync()) {
    ...
    if (isAsyncDeclContext) {
      if (const ValueDecl *vd = dyn_cast<ValueDecl>(D)) {
        ...
        D->getASTContext().Diags.diagnose(
            D->getLoc(), diag::async_decl_must_be_available_from_async, D);
      }
    }
...
void AttributeChecker::visitFinalAttr(FinalAttr *attr) {
  ...
  if (auto accessAttr = D->getAttrs().getAttribute<AccessControlAttr>()) {
    if (accessAttr->getAccess() == AccessLevel::Open) {
      diagnose(attr->getLocation(), diag::open_decl_cannot_be_final, D);
      return;
    }
  }
...
void AttributeChecker::visitCustomAttr(CustomAttr *attr) {
  ...
  if (nominal->getAttrs().hasAttribute<PropertyWrapperAttr>()) {
    ...
    if (isa<ParamDecl>(D)) {
      ...
      auto *context = cast_or_null<ValueDecl>(D->getDeclContext()->getAsDecl());
      if (isa_and_nonnull<SubscriptDecl>(context)) {
        diagnose(attr->getLocation(),
                 diag::property_wrapper_param_not_supported, context);
        attr->setInvalid();
        return;
      }
    }
...
void AttributeChecker::checkBackDeployedAttrs(
  ...
  for (auto *Attr : Attrs) {
    ...
    if (isa<DestructorDecl>(D)) {
      diagnoseAndRemoveAttr(Attr, diag::attr_invalid_on_decl_kind, Attr, D);
      continue;
    }

    if (VD->isObjC()) {
      diagnoseAndRemoveAttr(Attr, diag::attr_incompatible_with_objc, Attr, VD);
      continue;
    }
      ...
      case AbstractFunctionDeclLookupErrorKind::CandidateTypeMismatch: {
        ...
        if (auto genSig = expectedOriginalFnType->getOptGenericSignature()) {
          diags.diagnose(invalidCandidate,
                         ...
                         invalidCandidate, expectedOriginalFnType,
